...
const BrandLogo = ({ size = "md", className = "", vertical = false }) => {
  ...
  return (
    <div className={`flex ${vertical ? 'flex-col items-center text-center' : 'flex-row items-center'} ${size === 'lg' || size === 'xl' ? 'gap-6 md:gap-8' : 'gap-3 md:gap-4'} ${className} max-w-full relative`}>
      <div className={`relative ${sizes[size]} aspect-square group flex items-center justify-center shrink-0 overflow-hidden rounded-full`}>
        ...
        <img 
          src="/logo.png" 
          alt="Trinetra Sentinel" 
          className="w-full h-full relative z-10 drop-shadow-[0_0_20px_rgba(0,207,255,0.8)] object-contain transition-all group-hover:scale-110 duration-500 rounded-full" 
        />
      </div>
      ...
      <div className="flex flex-col leading-[0.8] max-w-full">
        <span className={`${vertical ? 'text-xl md:text-2xl' : 'text-xl md:text-2xl'} font-display font-bold text-white tracking-tighter flex items-center gap-2 ${vertical ? 'justify-center' : ''}`}>
          Tri<span className="text-cyber-neon-blue">netra</span>
        </span>
